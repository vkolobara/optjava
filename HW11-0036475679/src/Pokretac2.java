...
import hr.fer.zemris.generic.ga.GAAlgorithm2;
...
//		String bestImage = "slika.png";
		...
		EVOThread thread = new EVOThread(new Runnable() {
			...
			@Override
			public void run() {
				GAAlgorithm alg = new GAAlgorithm2(min, max, max_iter, pop_size, N, merr, originalImage, bestImage);
				try {
	...
	private static void write(String path, int[] var) throws IOException {
		System.out.println("Zapisujem najbolje pronađene parametre u datoteku: " + path);
		BufferedWriter outputWriter = null;
		  ...
		  outputWriter.flush();  
		  outputWriter.close();  
		  System.out.println("Uspješno zapisao!");
	}
}
